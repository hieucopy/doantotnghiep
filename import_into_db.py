...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'my_telecom.settings')  # Thay 'my_telecom.settings' nếu tên module settings của bạn khác
django.setup()
...
# Xóa dữ liệu cũ trong các bảng (theo thứ tự ngược lại để tránh lỗi ràng buộc khóa ngoại)
...
for row in sqlite_cursor.fetchall():
    ...
    date_joined_dt = timezone.make_aware(datetime.datetime.fromisoformat(date_joined), timezone=pytz.UTC)
...
for row in sqlite_cursor.fetchall():
    ...
    created_at_dt = timezone.make_aware(datetime.datetime.fromisoformat(created_at), timezone=pytz.UTC)
    updated_at_dt = timezone.make_aware(datetime.datetime.fromisoformat(updated_at), timezone=pytz.UTC)
    mysql_cursor.execute(
...
for row in sqlite_cursor.fetchall():
    ...
    start_date_dt = timezone.make_aware(datetime.datetime.fromisoformat(start_date), timezone=pytz.UTC)
    end_date_dt = timezone.make_aware(datetime.datetime.fromisoformat(end_date), timezone=pytz.UTC)
    mysql_cursor.execute(
